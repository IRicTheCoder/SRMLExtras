...
namespace SRMLExtras.Templates
{
	public class FruitPlantableTemplate : ModPrefab<FruitPlantableTemplate>
	{
		...
		private Vector3 leavesPosition;
		private Vector3 leavesDeluxePosition;
		...
		public override FruitPlantableTemplate Create()
		{
			...
			).SetTransform(leavesPosition, Vector3.zero, Vector3.one);
			...
			if (isDeluxe)
			{
				...
				{
					doNotScaleAsReplacement = false
				}).SetTransform(new Vector3(3.8f, 0.8f, -3.8f), new Vector3(0, 225, 0), new Vector3(0.4f, 0.5f, 0.4f))
				.AddChild(leavesObj.Clone().SetTransform(leavesDeluxePosition, Vector3.zero, new Vector3(1.3f, 0.9f, 1.3f))));

				mainObject.AddChild(treeObj.Clone().AddComponents(new ScaleMarker()
				{
					doNotScaleAsReplacement = false
				}).SetTransform(new Vector3(-3.8f, 0.8f, 3.8f), new Vector3(0, 45, 0), new Vector3(0.4f, 0.5f, 0.4f))
				.AddChild(leavesObj.Clone().SetTransform(leavesDeluxePosition, Vector3.zero, new Vector3(1.3f, 0.9f, 1.3f))));
			}
