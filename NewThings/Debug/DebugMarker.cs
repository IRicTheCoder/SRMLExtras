...
namespace SRMLExtras.Debug
{
	public class DebugMarker : MonoBehaviour
	{
		...
		public void Start()
		{
			...
			// Make Area Marker
			if (BaseObjects.markerAreaMaterials.ContainsKey(type))
			{
				areaMarker = new GameObject("AreaMarker");
				areaMarker.transform.parent = transform;
				areaMarker.transform.localPosition = Vector3.zero;

				areaMarker.AddComponent<MeshFilter>().sharedMesh = BaseObjects.cubeMesh;
				areaMarker.AddComponent<MeshRenderer>().sharedMaterial = BaseObjects.markerAreaMaterials[type];
